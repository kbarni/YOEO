...
class Labelme2YOLO(object):
    ...
    def getclassid(self,cls):
        num_cls=3
        if cls=='FD leaf':
            num_cls=0
        if cls == "feuille fd ratee":
            num_cls=0
        if cls == "feuille symptomatique" :
            num_cls=0

        if cls == "ESCA tres precoce":
            num_cls=1
        if cls == "ESCA legerement marquee":
            num_cls=1
        if cls == "feuille ESCA":
            num_cls=1

        if cls == "facteur confondant":
            num_cls=2
        if cls == "facteur confondant +":
            num_cls=2
    ...
    def _get_circle_shape_yolo_object(self, shape, img_h, img_w):
        ...
        label_id = self.getclassid(shape['label'])
